...
public class TelnetFile {
    ...
    public static String fileLoad(String url, File file){
        ...
        if (url == null || "".equals(url)){
            throw new BusiException("WebConstants.REQUEST_PARAMETER_ERROR","参数不合法");
        }
        if (!file.exists()){
            throw new BusiException("WebConstants.ERROR","上传指定的文件不存在!");
        }
        ...
        try {
            ...
            if(status == HttpStatus.SC_OK){
                ...
                throw new BusiException("WebConstants.UPLOAD_FILE_FAILED","文件上传失败");
            }
        } catch (FileNotFoundException e){
            throw new BusiException("WebConstants.ERROR","上传的文件不存在",e.getMessage());
        } catch (IOException e){
            throw new BusiException("WebConstants.UPLOAD_FILE_FAILED","文件上传异常",e.getMessage());
        } finally{
//            释放链接
