...
class SimpleMonthView extends View
{
    ...
    private boolean sky2 = true;
    private boolean sky3 = false;
	...
	private void drawMonthTitle(Canvas canvas) {
        ...
        int mX = paddingDay * (findDayOffset() * 2 + 1) + mPadding;
        int mY = (mRowHeight + MINI_DAY_NUMBER_TEXT_SIZE) / 2 - DAY_SEPARATOR_WIDTH + MONTH_HEADER_SIZE / 5 * 2;

        StringBuilder stringBuilder = new StringBuilder(getMonthString().toLowerCase());
        stringBuilder.setCharAt(0, Character.toUpperCase(stringBuilder.charAt(0)));
        canvas.drawText(stringBuilder.toString(), mX, mY, mMonthTitlePaint);
	}
    ...
    //绘制星期
    private void drawWeekTitle(Canvas canvas,int x,int y,int  i) {
        int paddingDay = (mWidth - 2 * mPadding) / (2 * mNumDays);
        int mX = paddingDay * (findDayOffset() * 2 + 1) + mPadding;
        int mY = (mRowHeight + MINI_DAY_NUMBER_TEXT_SIZE) / 2 - DAY_SEPARATOR_WIDTH + MONTH_HEADER_SIZE / 5 * 2;
        mY = mY - 20;
        StringBuilder stringBuilder = new StringBuilder(getWeekString(i).toLowerCase());
        stringBuilder.setCharAt(0, Character.toUpperCase(stringBuilder.charAt(0)));
        canvas.drawText(stringBuilder.toString(), x, mY, mMonthTitlePaint);
    }
    ...
    private String getWeekString(int i) {
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat(week[i]+"");
        String monthTitleText = dateFormat.format(mCalendar.getTime());
        return monthTitleText;
    }
	...
	protected void drawMonthNums(Canvas canvas) {
		...
		int day = 1;
        int stampDayOffset = 0;

		while (day <= mNumCells) {
            int x = paddingDay * (1 + dayOffset * 2) + mPadding;
            stampDayOffset = (1 + dayOffset * 2);
...
//                }
            ...
            if (sky2){
                ...
                if (sky){
                    ...
                    if (GaHeApplication.weekList.size()==7){
                        GaHeApplication.weekList.clear();
                    }
                    drawWeekTitle(canvas,x,0,GaHeApplication.weekList.size());
                    Integer dotsCount = eventSymbols.get(day);
                    if (dotsCount != null && dotsCount > 0) {
                        drawDots(x, y, dotsCount, canvas);
                    }
                    GaHeApplication.weekList.add(x);
                }
                if (stampDayOffset == 13 && sky3) {
                    sky = false;
                    sky2 = false;
                }
            }
	...
	protected void initView() {
        ...
        mSelectedCirclePaint.setColor(mSelectedDaysColor);//0x00000
	...
	protected void onDraw(Canvas canvas) {
        ...
        /**进来绘制星期天数*/
        List<Integer> weekList = GaHeApplication.weekList;
                if (weekList.size() == 7){
                    for (int i = 0; i <weekList.size(); i++) {
                        drawWeekTitle(canvas,weekList.get(i),0,i);
                    }
                }
	}
